    ...
    public class ElasticSearchManager : IElasticSearch
    {
        ...
        public ElasticSearchManager(IConfiguration configuration)
        {
            ...
            _connectionSettings = new ConnectionSettings(pool,
                (builtInSerializer, connectionSettings) => new JsonNetSerializer(
        ...
        public async Task<IElasticSearchResult> CreateNewIndexAsync(IndexModel indexModel)
        {
            ...
            CreateIndexResponse? response = await elasticClient.Indices.CreateAsync(indexModel.IndexName,
                se => se.Settings(a => a.NumberOfReplicas(indexModel.NumberOfReplicas)
        ...
        public async Task<List<ElasticSearchGetModel<T>>> GetAllSerch<T>(SearchParameters parameters) where T : class
        {
            Type type = typeof(T);
